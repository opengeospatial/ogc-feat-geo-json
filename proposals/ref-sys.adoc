...
* The keys <<coord-ref-systems-schema,`coord-ref-sys`>> can be used to assert coordinate reference system information at the collection, feature or value levels.
...
** **Decision**: No.  Since the proposal allows CRS values to be included in-line, any reference system that the encoding format supports can be specified.  Something like OGCWKT or PROJJSON handle geographic, projected, vertical, temporal, etc. coordinate reference systems.
