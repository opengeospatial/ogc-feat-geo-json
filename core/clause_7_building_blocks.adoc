...
This Clause describes the JSON building blocks required to extend GeoJSON features and feature collections. The following clauses specify the formal requirements.
...
To indicate that a JSON document contains JSON-FG objects and to indicate the JSON-FG version and requirements classes implemented in the document, a `conformsTo` member is required to be included in the root object of the JSON document.
...
In this Standard, a new key "coordRefSys" is defined and can be used to assert the CRS of a JSON-FG geometry at the feature collection, feature, or geometry levels.
...
GeoJSON (both the current <<rfc7946,RFC>> and the https://geojson.org/geojson-spec.html[obsolete version]) fixed the CRS for geometries to the "WGS84 datum, and with [an axis order of] longitude and latitude [and coordinate] units of decimal degrees".  The https://geojson.org/geojson-spec.html[obsolete version] supported other CRSs through defining the key "crs" for specifying the CRS.  In the https://tools.ietf.org/html/rfc7946[RFC] the "crs" member was removed, but a _prior arrangement_ provision was added that if all parties agree, coordinates could be represented in a different reference system. The result is that there is no interoperable way to unambiguously specify a different CRS in GeoJSON.  As such, the only safe approach is to continue using OGC:CRS84(h) for GeoJSON and ignore the _prior arrangement_ provision and the old "crs" key.
...
Additional JSON-FG building blocks like the "place" member are not bound by the restrictions in GeoJSON and so this Standard provides for handling CRSs in JSON-FG objects in a way that does not interfere with anything, past or present, defined in any of the GeoJSON specifications. The GeoJSON building blocks can continue to operate as always, but JSON-FG building blocks provides enhanced CRS support.
...
If a geometry includes measure values, each position of the geometry has to contain a measure value. The number of cordinates in each position is the dimension of the CRS in "coordRefSys" plus one for the measure value.

The presence of measure values in a geometry is indicated by a "measures" member.

The value of the "measures" member is either a boolean value or an object with the following properties:

* "unit": The unit of the measure of the measure values. The property is required.
* "description": An optional description of the meaning of the measure values.

If the value of the "measures" member is a boolean value, the value `true` indicates that the geometry has measure values without specifying information about the semantics of the the measure values. The value `false` indicates that the geometry does not have measure values.

If the value of the "measures" member is an object, the geometry has measure values.
...
For a feature collection where all features have the same "featureType" value, the "featureType" member can and should be added once for the feature collection. The "featureType" member can then be omitted in the features. Declaring the feature type(s) once signals to clients that the feature collection is homogeneous with respect to the type, which clients can use to optimize their processing.
...
The same resource can be represented in the same format or media type, but with variations depending on the intended use of the data. Named variations are called profiles.

A profile does not alter the semantics of the resource representation itself, but to allow clients to learn about additional semantics that are associated with the resource representation, in addition to those defined by the media type.

This Standard leverages the concept of profiles to define a set of named variations of GeoJSON:
...
This allows that all variations can use "application/geo+json" as the media type for JSON-FG documents, too, but still distinguish the variations. 
