...
If a persistent resource exists, such as in a registry, that describes a feature type, a link to that resource with link relation type "type" can be added. In the case of multiple feature types per feature, multiple links are added.
