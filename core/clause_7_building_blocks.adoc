...
Only the first two coordinates are used to determine the circular arc (that is, the center and radius). The third coordinate is linearly interpolated along the arc.
...
In JSON-FG, a feature is an instance of a single feature type. If a feature is associated with multiple feature types, the primary type should be identified. 
