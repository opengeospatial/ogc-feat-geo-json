...
If a geometry includes measure values, each position of the geometry has to contain a measure value (or m coordinate). The number of cordinates in each position is the dimension of the CRS in "coordRefSys" plus one for the measure value.
...
* "unit": The unit of the measure of the measure values. The property is optional. It only applies if the value of the "enabled" property is `true`. 
